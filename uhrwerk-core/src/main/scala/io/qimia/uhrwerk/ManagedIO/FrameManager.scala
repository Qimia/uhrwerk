...
import io.qimia.uhrwerk.models.config.{Connection, Dependency, InTable, Source, Target}
...
trait FrameManager {
  ...
  def loadDataFrame[T <: InTable](conn: Connection, locationInfo: T, startTS: Option[LocalDateTime] = Option.empty): DataFrame
