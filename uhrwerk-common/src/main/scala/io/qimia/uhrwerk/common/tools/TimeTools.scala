...
object TimeTools {
  ...
  def getRangeFromAggregate(
    ...
    if (isHour) {
      ...
      val tsDay = ts
        .minusHours(ts.getHour)
        .minusMinutes(ts.getMinute)
        .minusSeconds(ts.getSecond)
      (tsDay, tsDay.plusDays(1))
    } else { // isMonth
      val tsMonth = ts
        .minusDays(ts.getDayOfMonth - 1)
        .minusHours(ts.getHour)
        .minusMinutes(ts.getMinute)
        .minusSeconds(ts.getSecond)
  ...
  def divisibleBy(big: Duration, small: Duration): Boolean =
    big.toMinutes % small.toMinutes == 0
  ...
  def dateTimeToPostFix(date: LocalDateTime, duration: Duration): String = {
    ...
    val batch =
      TimeTools.getBatchInHourNumber(date.toLocalTime, duration).toString
  ...
  def checkIsSequentialIncreasing(
    ...
    in.sliding(2)
      .map(pairSeq => Duration.between(pairSeq.head, pairSeq.tail.head))
      .forall(dur => dur == partitionSize)
  }
