...
class TimeToolsTest extends AnyFlatSpec {
  ...
  "A course datetime list filtered with a fine datetime list" should "only return values when the full duration is covered" in {
    ...
    val courseResultList = TimeTools.filterBySmallerBatchList(courseListA, courseDuration, dividerA, fineListA.toSet)
    ...
    val outB = TimeTools.filterBySmallerBatchList(courseListB, courseDuration, dividerB, fineListB.toSet)
    outB
  ...
  "Creating a windowed batch list and reversing it" should "return the original batchlist and have the right size" in {
    ...
    assert(windowedVersion.head === LocalDateTime.of(2010, Month.MARCH, 5, 7, 0, 0))
  ...
  "converting a time into a batch number" should "work for common minute values" in {
    ...
    val tenMinuteResult = tenMinuteTest.map(time => TimeTools.getBatchInHourNumber(time, Duration.ofMinutes(10)))
    ...
    val quarterMinuteResult = quarterTest.map(time => TimeTools.getBatchInHourNumber(time, Duration.ofMinutes(15)))
    ...
    val halfResult = halfTest.map(time => TimeTools.getBatchInHourNumber(time, Duration.ofMinutes(30)))
  ...
  //    val (startShouldBe, endShouldBe) = (LocalDateTime.of(2020, 1, 1, 8, 0), LocalDateTime.of(2020, 1, 1, 9, 0))
  //
  ...
  //    val (startDayShouldBe, endDayShouldBe) = (LocalDateTime.of(2020, 1, 1, 0, 0), LocalDateTime.of(2020, 1, 2, 0, 0))
  //
  ...
  //
  //    val (startMonthShouldBe, endMonthShouldBe) =
  ...
  //    val (startHourShouldBe, endHourShouldBe) =
  //      (LocalDateTime.of(2020, 1, 1, 6, 45), LocalDateTime.of(2020, 1, 1, 9, 45))
  //
  ...
  //    val (startDayShouldBe, endDayShouldBe) =
  //      (LocalDateTime.of(2019, 12, 31, 9, 45), LocalDateTime.of(2020, 1, 1, 9, 45))
  //
  //    assert(startDay === startDayShouldBe)
  //    assert(endDay === endDayShouldBe)
  //
  //    val (startMonthShouldBe, endMonthShouldBe) =
  //      (LocalDateTime.of(2019, 12, 2, 8, 45), LocalDateTime.of(2020, 1, 2, 8, 45))
  //
