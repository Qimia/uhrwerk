...
object DagTaskBuilder2 {
  ...
  /** Apply bulk optimization for a Dag on the level of a single table.
    ...
    */
  def bulkOptimizeTable(
      tableWrapper: TableWrapper,
      tasks: List[(DagTask2Key, DagTask2)]
  ): List[(DagTask2Key, DagTask2)] = {
    val sortedList =
      tasks.sortWith((a: (DagTask2Key, DagTask2), b: (DagTask2Key, DagTask2)) =>
        a._1.partition.isBefore(b._1.partition)
      )
    val bulkTimeSplits = TimeHelper.groupSequentialIncreasing(
      sortedList.map(_._1.partition),
      tableWrapper.tableDuration,
      tableWrapper.wrappedTable.getMaxBulkSize
    )
    val res: mutable.ListBuffer[(DagTask2Key, DagTask2)] =
  ...
  /** Apply a bulk-optimization step to the DagTask-map. This will put together tasks which can be combined according
    ...
    */
  def bulkOptimizeTaskmap(
      taskmap: Map[DagTask2Key, DagTask2]
  ): Map[DagTask2Key, DagTask2] = {
    val tableGroups = taskmap.toList
      .groupBy((tup: (DagTask2Key, DagTask2)) =>
        Environment.getTableIdent(tup._2.tableWrapper.wrappedTable)
      )
      .values
  ...
  /** Merge task lists together for when multiple output tables need to be combined in a single dag.
    ...
    */
  def mergeTaskLists(
      taskmapA: Map[DagTask2Key, DagTask2],
      taskmapB: Map[DagTask2Key, DagTask2]
  ): Map[DagTask2Key, DagTask2] = {
    val unpartitionedTimeA = taskmapA.values
      .filterNot(task => task.tableWrapper.wrappedTable.getPartitioned)
      .head
      .partitions
      .head
    ...
    BParts
      ...
      .head
      ._2
      .foreach((kv: (DagTask2Key, DagTask2)) => {
    ...
    val BNotFoundByPartition = notFoundParts.groupBy(kv =>
      kv._2.tableWrapper.wrappedTable.getPartitioned
    )

    val unpartitionedUpdates = BNotFoundByPartition
      .map(partTuple => {
        if (partTuple._1) {
          partTuple._2
        } else {
...
class DagTaskBuilder2(environment: Environment) {
  ...
  /** Create a map with all DagTasks and their dependencies. It does not do any optimization on this dag yet
    ...
    */
  def buildTaskListFromTable(
      outTable: TableWrapper,
    ...
    def recursiveBuild(
        targetTaskKey: DagTask2Key,
        upstreamTask: Option[DagTask2Key],
        mapState: Map[DagTask2Key, DagTask2]
    ): Map[DagTask2Key, DagTask2] = {
      ...
      if (mapState.contains(targetTaskKey)) {
        ...
        val missingDeps: mutable.Set[DagTask2Key] =
          wrap.wrappedTable.getDependencies
            .map(d => {
              val ident = new TableIdent(
                d.getArea,
                d.getVertical,
                d.getTableName,
                d.getVersion
        ...
        val newDT2 = new DagTask2(
          wrap,
          ListBuffer(targetTaskKey.partition),
          missingDeps,
          upstreamDependants
        )
        ...
        if (missingDeps.nonEmpty) {
          missingDeps
            .map((k: DagTask2Key) =>
              (inMap: Map[DagTask2Key, DagTask2]) =>
                recursiveBuild(k, Option(targetTaskKey), inMap)
            )
    ...
    if (!outTable.wrappedTable.getPartitioned) {
      return recursiveBuild(
        DagTask2Key(getTableIdent(outTable.wrappedTable), callTime),
        Option.empty,
    ...
    val processedPartitions = environment.metaStore.tableDependencyService
      .processingPartitions(
        outTable.wrappedTable,
        partitionTs.asJava,
        mutable.HashMap[String, AnyRef]().asJava
      )
      .getProcessedTs
      .toSet
    ...
    val partitionToDoTs =
      partitionTs.filter(p => !processedPartitions.contains(p))
    val currentKeys = partitionToDoTs.map(t =>
      DagTask2Key(Environment.getTableIdent(outTable.wrappedTable), t)
    )
    val emptyMap = new immutable.HashMap[DagTask2Key, DagTask2]
    currentKeys
      .map((k: DagTask2Key) =>
        (inMap: Map[DagTask2Key, DagTask2]) =>
          recursiveBuild(k, Option.empty, inMap)
      )
