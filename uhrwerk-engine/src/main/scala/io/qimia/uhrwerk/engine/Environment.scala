...
class Environment(store: MetaStore, frameManager: FrameManager) {
   ...
   */
  def addConnections(connConfigLoc: String): Unit = {
    val connections = configReader.readConnections(connConfigLoc)
    connections.foreach(conn => store.connectionService.save(conn, true))
   ...
   */
  def addTable(tableConfigLoc: String, userFunc: TaskInput => DataFrame): Option[TableWrapper] = {
    val tableYaml = configReader.readTable(tableConfigLoc)
    val storeRes = store.tableService.save(tableYaml, true)
    // TODO: For now everything is overwrite
   ...
   */
  def setupDag(dagConfigLoc: String, userFuncs: Map[Ident, TaskInput => DataFrame]): Unit = {
    ...
    dagYaml.getConnections.foreach(conn => store.connectionService.save(conn, true))
    dagYaml.getTables.foreach(t => {
      val ident = TableIdent(t.getArea,
        t.getVertical,
        t.getName,
        t.getVersion)
